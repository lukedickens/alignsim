...
def run_fixed_variance_latent_facility_experiment(means, fixed_var, N=1000, n_trials=50):
    """
...
def run_fixed_evidence_latent_facility_experiment(means, fixed_evidence, N=1000, n_trials=50):
    """
    Runs a simulation sweeping through mean accuracies with fixed variance.
    Calculates Error Consistency (Kappa) for shared item difficulty.
    """
    ...
    for mu in means:
        try: 
            dist = get_beta_dist_from_evidence(mu, fixed_evidence)
        except e:
            print(f"error {e}")
